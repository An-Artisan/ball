...
class CreateOpenBall extends Command
{
    ...
    private function doRun()
    {
        ...
        try {
            ...
            if ((($currentBet->start_time) + $currentBet->current_open_ball_time) <= time()) {
                ...
                $currentBet->first_ball = $ball['ball'][0];
                $currentBet->second_ball = $ball['ball'][1];
                $currentBet->third_ball = $ball['ball'][2];
                $currentBet->fourth_ball = $ball['ball'][3];
                $currentBet->fifth_ball = $ball['ball'][4];
                $currentBet->win_or_lose = $ball['money'];
                $currentBet->save();
                $this->createNextBall($currentBet);
                $this->userBetSettle($ball['ball'],$userBetAll,$userBetOdds);
            }
    ...
    private  function userBetSettle($ball,$userBetAllObject,$userBetOddsObject) {
        ...
        foreach ($userBetAllObject as $item) {
            ...
            foreach ($itemArray as $key => $value) {
                if (array_key_exists($key, $userBetOdds)) {
                    ...
                    $rules[$key]["bet_odds_price"] = bcmul($value, $userBetOdds[$key], 2);
                }
            }
            $result = -(money($ball, $rules));
        ...
        foreach ($userBetAll as $item) {
            foreach ($item as $key => $value) {
                if (array_key_exists($key, $userBetOdds)) {
                    ...
                    $rules[$key]["bet_price"] = bcadd($rules[$key]["bet_price"], $value, 2);
                    $rules[$key]["bet_odds_price"] = bcadd($rules[$key]["bet_odds_price"], bcmul($value, $userBetOdds[$key], 2), 2);
                }
            }
        }
        $firsts = range(0, 9);
